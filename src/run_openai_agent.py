"""Run the real OpenAI finance agent on settlement exceptions."""
...
from openai_agent import investigate


def main() -> None:
    parser = argparse.ArgumentParser(description="Run real OpenAI finance agent")
    parser.add_argument("--data-dir", type=Path, default=Path("data"))
    parser.add_argument("--results-dir", type=Path, default=Path("results"))
    parser.add_argument("--limit", type=int, default=10, help="Number of exceptions to send to the LLM")
    ...
    for index, settlement_id in enumerate(exception_ids, 1):
        try:
            ...
            print(f"[{index}/{len(exception_ids)}] {settlement_id} -> {decision['root_cause']} ({decision['confidence']:.2f}) tools={decision['tool_calls']}")
        except Exception as exc:
            failures.append({"case_id": settlement_id, "error": str(exc)})
            print(f"[{index}/{len(exception_ids)}] {settlement_id} -> ERROR: {exc}")

    args.results_dir.mkdir(parents=True, exist_ok=True)
    (args.results_dir / "openai_agent_decisions.json").write_text(json.dumps(decisions, indent=2), encoding="utf-8")
    (args.results_dir / "openai_agent_failures.json").write_text(json.dumps(failures, indent=2), encoding="utf-8")
    ...
    (args.results_dir / "openai_agent_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
